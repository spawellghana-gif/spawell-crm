...
begin
  ...
  if trim(coalesce(p_project_id, '')) = '' then
    raise exception 'Service account project_id is required.' using errcode = '22023';
  end if;
...
begin
  ...
  configured := coalesce(v_secret ? 'project_id', false)
    and coalesce(v_secret ? 'client_email', false)
    and coalesce(v_secret ? 'private_key', false)
    and coalesce(length(v_secret->>'project_id') > 0, false)
...
revoke all on function public.set_google_ads_service_account(text,text,text) from public;
grant execute on function public.set_google_ads_service_account(text,text,text) to authenticated, service_role;
