...
export function googleAdsAuthMode(): GoogleAdsAuthMode {
  if (env("VERCEL") === "1" || env("VERCEL_ENV") || env("VERCEL_OIDC_TOKEN")) return "vercel_oidc";
...
export function googleAdsConfigured(): boolean { return googleAdsMissing().length === 0; }
...
export const sha256Hex = (value: string): string => value ? createHash("sha256").update(value, "utf8").digest("hex") : "";
...
async function oidcAccessToken(subjectToken: string): Promise<{ value: string; expiresAt: number }> {
  ...
  const impersonateRes = await fetch(`https://iamcredentials.googleapis.com/v1/projects/-/serviceAccounts/${serviceAccount}:generateAccessToken`, {
    method: "POST",
    headers: { authorization: `Bearer ${stsBody.access_token}`, "content-type": "application/json", "x-goog-user-project": GOOGLE_ADS_WIF.projectId },
    body: JSON.stringify({ scope: [SCOPE], lifetime: "3600s" }),
    signal: AbortSignal.timeout(15_000),
  });
  const impersonateBody = await impersonateRes.json().catch(() => ({} as any));
  if (!impersonateRes.ok || !impersonateBody.accessToken) throw new Error(`Google service-account impersonation failed: ${googleError(impersonateBody, String(impersonateRes.status))}`);
  const expiresAt = impersonateBody.expireTime ? Date.parse(impersonateBody.expireTime) : Date.now() + 55 * 60_000;
...
async function keyAccessToken(c: GoogleAdsConfig): Promise<{ value: string; expiresAt: number }> {
  ...
  const b64 = (v: string) => Buffer.from(v).toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
  const unsigned = `${b64(JSON.stringify({ alg: "RS256", typ: "JWT" }))}.${b64(JSON.stringify({ iss: c.clientEmail, scope: SCOPE, aud: "https://oauth2.googleapis.com/token", exp: now + 3600, iat: now }))}`;
  const signer = createSign("RSA-SHA256"); signer.update(unsigned);
  const signature = signer.sign(c.privateKey).toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
  const res = await fetch("https://oauth2.googleapis.com/token", { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ grant_type: "urn:ietf:params:oauth:grant-type:jwt-bearer", assertion: `${unsigned}.${signature}` }), signal: AbortSignal.timeout(15_000) });
  const body = await res.json().catch(() => ({} as any));
  if (!res.ok || !body.access_token) throw new Error(`Google refused the service account key: ${googleError(body, String(res.status))}`);
  return { value: body.access_token, expiresAt: Date.now() + (body.expires_in ?? 3600) * 1000 };
}
...
export type ConversionPayload = { transactionId: string; conversionAt: string; valuePesewas: number; currency: string; gclid?: string; gbraid?: string; wbraid?: string; emailSha256?: string; phoneSha256?: string; };
export type IngestResult = { ok: boolean; httpStatus: number; response: unknown; error?: string; };

export async function sendConversion(payload: ConversionPayload, opts: { validateOnly?: boolean } = {}): Promise<IngestResult> {
  const c = googleAdsConfig();
  const missing = googleAdsMissing();
  if (missing.length) return { ok: false, httpStatus: 0, response: null, error: `Configuration required: ${missing.join(", ")}` };
  ...
  if (!Object.keys(adIdentifiers).length && !identifiers.length) return { ok: false, httpStatus: 0, response: null, error: "No ad identifier and no customer match data — nothing to attribute." };
  const body = {
    destinations: [{ operatingAccount: { accountType: "GOOGLE_ADS", accountId: c.customerId }, ...(c.loginCustomerId ? { loginAccount: { accountType: "GOOGLE_ADS", accountId: c.loginCustomerId } } : {}), productDestinationId: c.conversionAction }],
    encoding: "HEX",
    events: [{ eventTimestamp: payload.conversionAt, ...(Object.keys(adIdentifiers).length ? { adIdentifiers } : {}), conversionValue: payload.valuePesewas / 100, currency: payload.currency, transactionId: payload.transactionId, eventSource: "OTHER", ...(identifiers.length ? { userData: { userIdentifiers: identifiers } } : {}), consent: { adPersonalization: "CONSENT_STATUS_UNSPECIFIED", adUserData: "CONSENT_STATUS_UNSPECIFIED" } }],
    validateOnly: Boolean(opts.validateOnly),
  };
  try {
    const token = await accessToken();
    const res = await fetch(INGEST_URL, { method: "POST", headers: { authorization: `Bearer ${token}`, "content-type": "application/json", "x-goog-user-project": c.quotaProjectId }, body: JSON.stringify(body), signal: AbortSignal.timeout(15_000) });
    const response = await res.json().catch(() => null);
    if (!res.ok) { const message = (response as any)?.error?.message ?? `Google returned ${res.status}`; return { ok: false, httpStatus: res.status, response, error: message }; }
    const failures = (response as any)?.errors ?? (response as any)?.eventErrors;
    if (Array.isArray(failures) && failures.length) return { ok: false, httpStatus: res.status, response, error: JSON.stringify(failures).slice(0, 500) };
    if (!opts.validateOnly && !(response as any)?.requestId) return { ok: false, httpStatus: res.status, response, error: "Google returned no upload request ID." };
    return { ok: true, httpStatus: res.status, response };
  } catch (e) { return { ok: false, httpStatus: 0, response: null, error: e instanceof Error ? e.message : "Request failed" }; }
}
